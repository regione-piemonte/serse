...
public interface ParentDAO {
	...
	@Select("select coalesce(max(numerazione_destinatario),0)+1 from serse_r_destinatario_servizio where id_servizio = #{idServizio}")
    Long getNextNumerazioneDestinatario(Long idServizio);
	...
	@Select("select nextval('seq_serse_t_incontro')")
	Long getNextValIdIncontro();

	@Select("select nextval('seq_serse_t_luogo_incontro')")
	Long getNextValIdLuogoIncontro();
}
