...
import it.csi.serse.serseapi.api.exception.UtenteNonAbilitatoException;
...
public class SerseApiExceptionHandler implements ExceptionMapper<RuntimeException>{
	...
	private Response handleNotFoundException(NotFoundException exception) {
		return Response.status(Status.BAD_REQUEST).entity(ErrorHelper.createErrore(HttpStatus.BAD_REQUEST.value(),exception.getMessage(), MSG_BAD_REQUEST)).build();
	}
	
	private Response handleNotAllowedException(NotAllowedException exception) {
		return Response.status(Status.METHOD_NOT_ALLOWED).entity(ErrorHelper.createErrore(HttpStatus.METHOD_NOT_ALLOWED.value(), exception.getMessage())).build();
	}
	
	private Response handleDataAccessException(DataAccessException exception) {
		return Response.status(Status.INTERNAL_SERVER_ERROR).entity(ErrorHelper.createErrore(HttpStatus.INTERNAL_SERVER_ERROR.value(), MSG_ERROR)).build();
	}
	
	private Response handleBadSqlGrammarException(BadSqlGrammarException exception) {
		return Response.status(HttpStatus.INTERNAL_SERVER_ERROR.value()).entity(ErrorHelper.createErrore(HttpStatus.INTERNAL_SERVER_ERROR.value(), MSG_ERROR)).build();
	}
	
	
	private Response handleErroreGestitoException(ErroreGestitoException exception) {
		return Response.status(HttpStatus.INTERNAL_SERVER_ERROR.value()).entity(ErrorHelper.createErrore(HttpStatus.INTERNAL_SERVER_ERROR.value(), exception.getMessage())).build();
	}

	private Response handleUtenteNonAbilitatoException(UtenteNonAbilitatoException exception) {
		return Response.status(HttpStatus.FORBIDDEN.value()).entity(ErrorHelper.createErrore(HttpStatus.FORBIDDEN.value(), MSG_FORBIDDEN_USER)).build();
	}

	private Response handleGenericException(RuntimeException exception) {
		return Response.status(HttpStatus.INTERNAL_SERVER_ERROR.value()).entity(ErrorHelper.createErrore(HttpStatus.INTERNAL_SERVER_ERROR.value(), MSG_UNEXPECTED_ERROR)).build();
	}
